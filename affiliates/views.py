...
class AffiliateDashboardView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        current_rank = (
            affiliate.rank.current_rank if hasattr(affiliate, "rank") else None
        )
        ...
        next_rank = (
            Rank.objects.filter(
                is_active=True,
                min_personal_referrals__gt=(
                    current_rank.min_personal_referrals if current_rank else 0
                ),
            )
        ...
        total_clicks = (
            AffiliateProductLink.objects.filter(affiliate=affiliate).aggregate(
        ...
        # Recent referrals
        recent_referrals = Affiliate.objects.filter(referred_by=affiliate).order_by(
            "-created_at"
        )[:5]
        # Default referral link
        referral_path = reverse("register_user")
        default_referral_link = (
            f"{settings.SITE_DOMAIN}{referral_path}?ref={affiliate.id}"
        )
        ...
        context.update(
            {
                ...
                "rank_logo": (
                    current_rank.logo.url
                    if current_rank and current_rank.logo
                    else None
                ),
                "rank_reward": rank_rewards,
                "next_rank": (
                    next_rank.title if next_rank else "No further ranks available"
                ),
                "next_rank_min_personal_referrals": (
                    next_rank.min_personal_referrals if next_rank else None
                ),
                "next_rank_min_total_referrals": (
                    next_rank.min_total_referrals if next_rank else None
                ),
                "product_commissions": product_commissions,
                "referral_commissions": referral_commissions,
                "total_earnings": total_earnings,
                "wallet_balance": wallet_balance,
                "total_products_sold": ProductPurchase.objects.filter(
                    affiliate=affiliate
                ).count(),
                "referrals": Affiliate.objects.filter(
                    referred_by=affiliate
                ),  # Downline tracking
                "personal_referrals": personal_referrals,
                "total_referrals": total_referrals,
                "affiliate_products": Product.objects.filter(
                    affiliateproductlink__affiliate=affiliate
                ),
                ...
                "sponsor_phone": (
                    sponsor.user.profile.phone_number
                    if sponsor and hasattr(sponsor.user, "profile")
                    else "N/A"
                ),
                "sponsor_location": (
                    f"{sponsor.user.profile.city}, {sponsor.user.profile.state}, {sponsor.user.profile.country}"
                    if sponsor and hasattr(sponsor.user, "profile")
                    else "N/A"
                ),
                "user_phone": (
                    affiliate.user.profile.phone_number
                    if hasattr(affiliate.user, "profile")
                    else "N/A"
                ),
                "user_location": (
                    f"{affiliate.user.profile.city}, {affiliate.user.profile.state}, {affiliate.user.profile.country}"
                    if hasattr(affiliate.user, "profile")
                    else "N/A"
                ),
...
def list_affiliate_links(request):
    ...
    return render(
        request,
        "affiliates/affiliate_links.html",
        {
            "links": links,
...
def view_genealogy(request):
    ...
    affiliate = request.user.affiliate
    referrals = Affiliate.objects.filter(referred_by=affiliate).select_related(
        "rank__current_rank"
    )
    ...
    genealogy = [
        {
            ...
            "rank": (
                referral.rank.current_rank.title
                if hasattr(referral, "rank") and referral.rank.current_rank
                else "No Rank"
            ),
    ...
    return render(
        request, "affiliates/genealogy.html", {"genealogy": genealogy, "ranks": ranks}
    )
...
def genealogy_tree_data(request):
    ...
    def build_tree(affiliate, level=0):
        ...
        color_class = (
            "top-node"
            if level == 0
            else "direct-referral-node" if level == 1 else "other-referral-node"
        )
