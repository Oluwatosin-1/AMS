...
class Affiliate(BaseModel):
    ...
    payout_threshold = models.DecimalField(
        max_digits=10, decimal_places=2, default=50.0
    )
    ...
    def get_performance_data(self):
        ...
        performance_data = (
            ProductPurchase.objects.filter(affiliate=self)
            .annotate(month=TruncMonth("purchased_at"))
            .values("month")
            .annotate(
                total_sales=Count("id"), total_commissions=Sum("commission_earned")
            )
            .order_by("month")
        )
    ...
    def calculate_total_earnings(self):
        ...
        product_commissions = (
            self.productpurchase_set.aggregate(total=Sum("commission_earned"))["total"]
            or 0
        )

        referral_commissions = (
            Referral.objects.filter(affiliate=self).aggregate(
...
class AffiliateEarning(models.Model):
    ...
    affiliate = models.ForeignKey(
        Affiliate, on_delete=models.CASCADE, related_name="earnings"
    )
