...
def referral_tree(request):
    ...
    referral_tree = [
        {
            ...
            "rank": (
                child.rank.current_rank.title
                if hasattr(child, "rank") and child.rank.current_rank
                else "No Rank"
            ),
    ...
    return render(
        request, "affiliates/referral_tree.html", {"referral_tree": referral_tree}
    )
...
def recent_referrals(request):
    ...
    affiliate = request.user.affiliate
    referrals = Affiliate.objects.filter(referred_by=affiliate).order_by("-created_at")[
        :10
    ]
